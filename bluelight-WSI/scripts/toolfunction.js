...
function removeWSI(num) {
    try {
        getByid("WSI_Canvas_" + num).width = 2;
        getByid("WSI_Canvas_" + num).height = 2;
        getByid("WSI_Canvas_" + num).class = "remove";
        getByid("WSI_Canvas_" + num).parentElement.removeChild(getByid("WSI_Canvas_" + num));
        delete getByid("WSI_Canvas_" + num);
    } catch (ex) {
        try {
            getByid("WSI_Canvas_" + num).parentElement.removeChild(getByid("WSI_Canvas_" + num));
            delete getByid("WSI_Canvas_" + num);
...
function swapESIId(directionNumber) {
    ...
    var FramwsArray = getWSIArray(Math.ceil(matrixColumns / width), Math.ceil(matrixRows / height));
    if (wsi_deep != 0) {
        var SopObject0 = sortSops[0];//Patient.Study[0].Series[0].Sop[0];

        var width0 = SopObject0.Frames[0].width;
        var height0 = SopObject0.Frames[0].height;
        var matrixColumns0 = SopObject0.matrixColumns;
        var matrixRows0 = SopObject0.matrixRows;
        var FramwsArray0 = getWSIArray(Math.ceil(matrixColumns0 / width0), Math.ceil(matrixRows0 / height0));
        var scaleX = parseFloat(matrixColumns0) / parseFloat(matrixColumns);
        var scaleY = parseFloat(matrixRows0) / parseFloat(matrixRows);
        //wsi_xy[0]=
    } else {
        var scaleX = 1;
        var scaleY = 1;
    ...
    for (var i = 0; i < 3; i++) {
        for (var i2 = 1; i2 <= 3; i2++) {
            IdArr.push(getByid("WSI_Canvas_" + (i * 3 + i2)));
        }
    }
    var IdArr = [];
    for (var i = 1; i <= 9; i++) {
        IdArr.push(getByid("WSI_Canvas_" + (i)));
    }
    if (directionNumber == 1) {
        ...
        for (var i = 1; i <= 9; i++) {
            try {
                IdArr[i - 1].id = "WSI_Canvas_" + (i + 4);
            } catch (ex) { }
        }
        wsi_xy[0] = wsi_xy[0] - parseInt(wsi_size * scaleX);
        wsi_xy[1] = wsi_xy[1] - parseInt(wsi_size * scaleY);
        GetViewport().newMousePointY -= parseInt(getByid("WSI_Canvas_5").style.height);
        GetViewport().newMousePointX -= parseInt(getByid("WSI_Canvas_5").style.width);
    }
...
function swapId(str1, str2) {
    var id1 = getByid(str1);
    var id2 = getByid(str2);
    var tempId = id1.id;
    id1.id = id2;
    id2.id = tempId;
}
...
function getCurrPoint(e) {
    var currX =e.pageX ;// parseFloat(parseFloat((e.pageX  /* - newMousePointX[viewportNumber]*/ - 0)) * (GetViewport().imageWidth / parseFloat(canvas.style.width)));
    var currY =e.pageY;// parseFloat(parseFloat((e.pageY /*- newMousePointY[viewportNumber] */ - 0)) * (GetViewport().imageHeight / parseFloat(canvas.style.height)));
    return [currX, currY];
}
...
function GetFrame2IndexFrowSop(frame, alt) {
    if (!alt) var alt = GetViewport().alt;
    for (var i = 0; i < Patient.StudyAmount; i++) {
        for (var j = 0; j < Patient.Study[i].SeriesAmount; j++) {
            for (var l = 0; l < Patient.Study[i].Series[j].SopAmount; l++) {
                if (Patient.Study[i].Series[j].Sop[l].SopUID == alt) {
                    for (var f = 0; f < Patient.Study[i].Series[j].Sop[l].FrameAmount; f++) {
                        if (Patient.Study[i].Series[j].Sop[l].Frames[f].Frame == frame) {
                            return [i, j, l, f];
                        }
                    }
                }
...
function SearchFrame2Index(alt, frame) {
    for (var i = 0; i < Patient.StudyAmount; i++) {
        for (var j = 0; j < Patient.Study[i].SeriesAmount; j++) {
            for (var l = 0; l < Patient.Study[i].Series[j].SopAmount; l++) {
                if (Patient.Study[i].Series[j].Sop[l].SopUID == alt) {
...
function sortSopsFromSize() {
    //let index = SearchUid2Index(alt);
    //let i = index[0],
    //    j = index[1],
    //    k = index[2];
    // if (Patient.Study[0].Series[0].Sop[0].SopUID == alt) {
    var list = [];
    // var Onum = parseInt(Patient.Study[i].Series[j].Sop[k].InstanceNumber);
    for (var l = 0; l < Patient.Study[0].Series[0].Sop.length; l++) {
        list.push(Patient.Study[0].Series[0].Sop[l]);
    }
    for (var m = 0; m < list.length; m++) {
        for (var n = 0; n < list.length; n++) {
            if (parseInt(list[m].matrixColumns * list[m].matrixRows) > parseInt(list[n].matrixColumns * list[n].matrixRows)) {
                var tempUID = list[m];
                list[m] = list[n];
                list[n] = tempUID;
            }
...
function sortFrames(alt) {
    ...
    if (Patient.Study[i].Series[j].Sop[k].SopUID == alt) {
        var list = [];
        // var Onum = parseInt(Patient.Study[i].Series[j].Sop[k].InstanceNumber);
        for (var l = 0; l < Patient.Study[i].Series[j].Sop[k].Frames.length; l++) {
            list.push(Patient.Study[i].Series[j].Sop[k].Frames[l]);
        }
        for (var m = 0; m < list.length; m++) {
            for (var n = 0; n < list.length; n++) {
                if (parseInt(list[m].Frame) < parseInt(list[n].Frame)) {
                    var tempUID = list[m];
                    list[m] = list[n];
                    list[n] = tempUID;
                }
