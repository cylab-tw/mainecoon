window.onload = function () {
  ...
  GetViewport().addEventListener('touchstart', Touchstart, false);
  GetViewport().addEventListener('touchmove', Touchmove, false);
  GetViewport().addEventListener('touchend', Touchend, false);
  GetViewport().style.overflow = "hidden";
  window.onresize();
}
...
function loadLdcmview() {
  ...
  for (var i = 0; i < Viewport_Total; i++) {
    ...
    NewDiv.canvas = function (num) {
      //if(!num) 
      //return getByid("WSI_Canvas_5");
      if (!num) num = 0;
      if (this.getElementsByClassName("cornerstone-canvas")[num])
        return this.getElementsByClassName("cornerstone-canvas")[num];
      else
        return null;
    }
    NewDiv.ctx = function (num) {
      if (!num) num = 0;
      if (this.getElementsByClassName("cornerstone-canvas")[num])
        return this.getElementsByClassName("cornerstone-canvas")[num].getContext("2d");
      else
        return null;
    }
...
function readConfigJson(url, callBack, callBack2) {
  ...
  request.onload = function () {
    ...
    tempConfig.FileType = tempDicomResponse["DICOM/JPEG"];
  ...
  request.onload = function () {
    ...
    for (let series = 0; series < DicomStudyResponse.length; series++) {
      ...
      SeriesRequest.onload = function () {
        let DicomResponse = SeriesRequest.response;
                 ...
                 // }
                  try {
                    ...
                    /*if (DicomResponse[i]["00200013"].Value[0] == min) {
                      ...
                      else if (ConfigLog.WADO.WADOType == "RS") WSI_Loader(url);
                    }
        ...
        for (var i = 0; i < DicomResponse.length; i++) {
          ...
          try {
            ...
            //預載入DICOM至Viewport
            if (ConfigLog.WADO.WADOType == "RS") WSI_Loader(url, DicomResponse.length);
            else {
              if (Hierarchy == 0)
