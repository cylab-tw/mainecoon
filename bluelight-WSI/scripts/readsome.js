...
function getWSIArray(xRect, yRect) {
  if (yRect == 0) xRect = 1;
  if (yRect == 0) yRect = 1;
  var arr = new Array(yRect);
  for (var i = 0; i < arr.length; i++) {
    arr[i] = new Array(xRect);
  }
  count = 1;
  for (var i = 0; i < arr.length; i++) {
...
function getPointFromWSIArray(arr, frame) {
  var count = 0;
  for (var i = 0; i < arr.length; i++) {
    for (var i2 = 0; i2 < arr[i].length; i2++) {
      count++;
...
function loadUID_WSI(wsi) {
  var Hierarchy = 0;
  var NumberOfStudy = -1;
  for (var i = 0; i < Patient.StudyAmount; i++) {
    if (Patient.Study[i].StudyUID == wsi.study)
      NumberOfStudy = i;
  }
  if (NumberOfStudy == -1) {
    var Study = {};
    Study.StudyUID = wsi.study;
    Study.PatientId = wsi.patientId;
    Study.SeriesAmount = 1;
    Study.Series = [];
    var Series = {};
    Series.SeriesUID = wsi.series;
    Series.SopAmount = 1;
    Series.Sop = [];
    var Sop = {};
    Sop.InstanceNumber = wsi.instance;
    Sop.SopUID = wsi.sop;
    Sop.response = wsi.response;
    Sop.FrameAmount = 1;
    Sop.Frames = [];
    Sop.matrixColumns = wsi.matrixColumns;
    Sop.matrixRows = wsi.matrixRows;
    Sop.instance_url = wsi.instance_url;
    Sop.Xoffset = wsi.Xoffset;
    Sop.Yoffset = wsi.Yoffset;
    Sop.FramwsArray = getWSIArray(parseInt(wsi.matrixColumns / wsi.width), parseInt(wsi.matrixRows / wsi.height));
    //Sop.imageId = wsi.imageId;
    var Frames = {};
    Frames.Frame = wsi.frame;
    Frames.imageId = wsi.imageId;
    Frames.x = wsi.x;
    Frames.y = wsi.y;
    Frames.width = wsi.width;
    Frames.height = wsi.height;
    Frames.rect = wsi.rect;
    Frames.dataURL = wsi.dataURL;

    Sop.Frames.push(Frames);
    Series.Sop.push(Sop);
    ...
    for (var i = 0; i < Patient.Study[NumberOfStudy].SeriesAmount; i++) {
      if (Patient.Study[NumberOfStudy].Series[i].SeriesUID == wsi.series)
        isSeries = i;
    }
    if (isSeries == -1) {
      var Series = {};
      ...
      for (var i = 0; i < Patient.Study[NumberOfStudy].Series[isSeries].SopAmount; i++) {
        if (Patient.Study[NumberOfStudy].Series[isSeries].Sop[i].SopUID == wsi.sop)
          isSop = i;
      }
      if (isSop == -1) {
        var Sop = {};
        ...
        Hierarchy = 3;
        var isFrame = -1;
        for (var i = 0; i < Patient.Study[NumberOfStudy].Series[isSeries].Sop[isSop].FrameAmount; i++) {
          if (Patient.Study[NumberOfStudy].Series[isSeries].Sop[isSop].Frames[i].Frame == wsi.frame)
            isFrame = i;
        }
        if (isFrame == -1) {
          //Sop.imageId = wsi.imageId;
          ...
          Frames.rect = wsi.rect;
          Frames.dataURL = wsi.dataURL;
          Frames.Xoffset = wsi.Xoffset;
          Frames.Yoffset = wsi.Yoffset;
          //Frames.matrixColumns = wsi.matrixColumns;
          //Frames.matrixRows = wsi.matrixRows;
          // Sop.push(Frames);
          Patient.Study[NumberOfStudy].Series[isSeries].Sop[isSop].Frames.push(Frames);
          Patient.Study[NumberOfStudy].Series[isSeries].Sop[isSop].FrameAmount += 1;
        } else {
          Hierarchy = -1;
        }
      }
