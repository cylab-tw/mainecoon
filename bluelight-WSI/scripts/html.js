...
function addBlockSize() {
  getByid("button_up").onclick = function () {
    loadNewWSI('up');
  };
  getByid("button_down").onclick = function () {
    loadNewWSI('down');
  };
  getByid("button_right").onclick = function () {
    loadNewWSI('right');
  };
  getByid("button_left").onclick = function () {
    loadNewWSI('left');
  };
  getByid("button_enlarge").onclick = function () {
    if (wsi_deep != 0)
      wsi_deep--;
    loadNewWSI();
    // displayWsiImage();
  };
  getByid("button_narrow").onclick = function () {
    if (wsi_deep != sortSopsFromSize().length - 1);
    wsi_deep++;
    loadNewWSI();
    //displayWsiImage();
  };
  let radio = getClass("block_radio");
  for (var i = 0; i < radio.length; i++) {
    radio[i].onchange = function () {
      wsi_global_width=0;
      wsi_global_height=0;
      if (getByid("block_1x1").checked == true) {
        wsi_size = 1;
        if (loadingWSI == 0) {
          loadingWSI = 11;
          if (wsi_deep < 0) wsi_deep = 0;
          removeWSI(1);
          removeWSI(2);
          removeWSI(3);
          removeWSI(4);
        ...
        //displayWsiImage(undefined);
      }
      else if (getByid("block_2x2").checked == true) {
        wsi_size = 2;
        if (loadingWSI == 0) {
          loadingWSI = 11;
          if (wsi_deep < 0) wsi_deep = 0;
      ...
      else if (getByid("block_3x3").checked == true) {
        wsi_size = 3;
        if (loadingWSI == 0) {
          loadingWSI = 11;
          if (wsi_deep < 0) wsi_deep = 0;
      ...
      else if (getByid("block_4x4").checked == true) {
        wsi_size = 4;
        if (loadingWSI == 0) {
          loadingWSI = 11;
          if (wsi_deep < 0) wsi_deep = 0;
      ...
      else if (getByid("block_5x5").checked == true) {
...
var timer = setInterval(function () {
  if(loadingWSI==0){
    getByid("stateId").innerHTML="State：Normal";
  }else{
    getByid("stateId").innerHTML="State：Loading";
  }
}, 500);
...
function imgInvalid(element) {
  if (openRendering) {
    ...
    if (element.classList.contains("WSI")) return false;
    else return true;
  }
